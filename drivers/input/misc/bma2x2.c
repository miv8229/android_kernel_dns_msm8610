...
struct bma2x2_data {
	...
	struct mutex read_write_mutex;//add by rongxiao.deng for mutex lock
				 ...
				 unsigned long event, void *data)
{
	...
	int *blank;
	struct bma2x2_data *bma222_data =
		container_of(self, struct bma2x2_data, fb_notif);
	if (evdata && evdata->data && event == FB_EVENT_BLANK &&
			bma222_data && bma222_data->bma2x2_client) {
		blank = evdata->data;
		if (*blank == FB_BLANK_UNBLANK)
		{  
            mutex_lock(&bma222_data->enable_mutex);
            bma222_data->in_suspend = 0;
	        if (atomic_read(&bma222_data->enable) == 1 ) {
		        bma2x2_set_mode(bma222_data->bma2x2_client, BMA2X2_MODE_NORMAL);
#ifndef CONFIG_BMA_ENABLE_NEWDATA_INT
		        schedule_delayed_work(&bma222_data->work,
				msecs_to_jiffies(atomic_read(&bma222_data->delay)));
#endif
		    }
            mutex_unlock(&bma222_data->enable_mutex);
		}
		else if (*blank == FB_BLANK_POWERDOWN)
		{ 
            mutex_lock(&bma222_data->enable_mutex);
                if (atomic_read(&bma222_data->enable) == 1) {
#if 0//modify by rongxiao.deng for pedometer apk
                    bma2x2_set_mode(bma222_data->bma2x2_client, BMA2X2_MODE_SUSPEND);
#ifndef CONFIG_BMA_ENABLE_NEWDATA_INT
                    cancel_delayed_work_sync(&bma222_data->work);
#endif
#endif
                }
                bma222_data->in_suspend = 1;
                mutex_unlock(&bma222_data->enable_mutex);
		}
	}
	return 0;
		...
		unsigned char reg_addr, unsigned char *data)
{
	...
	//modify(add) by junfeng.zhou.sz for fix i2c timeout error with 661727 begin . 20140501
    if(bma2x2->in_suspend_f)
    {
        printk(KERN_ERR "%s: devices in suspend\n", __func__);
        return -1;
    }
    //modify(add) by junfeng.zhou.sz for fix i2c timeout error with 661727 end . 
	mutex_lock(&bma2x2->read_write_mutex);
    //wake_lock_timeout(&bma2x2->wakelock, 3*HZ);
	dummy = i2c_smbus_read_byte_data(client, reg_addr);
	mutex_unlock(&bma2x2->read_write_mutex);
	if (dummy < 0)
		return -1;
		...
		u8 *data, u16 len)
{
    ...
    //modify(add) by junfeng.zhou.sz for fix i2c timeout error with 661727 end . 20140501
    mutex_lock(&bma2x2->read_write_mutex);
...
int accsns_get_acceleration_data(int *xyz)
{
    ...
    bma2x2_read_accel_xyz(gdata->bma2x2_client,
		gdata->sensor_type, &acc);
    xyz[0] = acc.x;
    xyz[1] = acc.y;
    xyz[2] = acc.z;
    return 0;
...
static void bma2x2_set_enable(struct device *dev, int enable)
{
	...
	if (enable) {
		...
		if (pre_enable == 1 && bma2x2->in_suspend == 0) {
			bma2x2_set_mode(bma2x2->bma2x2_client,
					BMA2X2_MODE_SUSPEND);
#ifndef CONFIG_BMA_ENABLE_NEWDATA_INT
...
//mark begin
void accsns_activate(int flgatm, int enable)
{  

   // if (enable != 0)
		enable = 1;
    bma2x2_set_enable(&(gdata->bma2x2_client->dev), enable);
}
...
static DEVICE_ATTR(value_cache, 0644,
...
static DEVICE_ATTR(fifo_data_out_frame, 0644,
...
static DEVICE_ATTR(enable_int, 0644,
		...
		const struct i2c_device_id *id)
{
	...
	mutex_init(&data->read_write_mutex);
    //wake_lock_init(&data->wakelock,WAKE_LOCK_SUSPEND, "bma2x2_wakelock");
	...
	err = request_irq(data->IRQ, bma2x2_irq_handler, IRQF_TRIGGER_RISING,
			"bma2x2", data);
	if (err)
		printk(KERN_ERR "%s could not request irq\n",__func__);
...
#endif
	atomic_set(&data->delay, BMA2X2_MAX_DELAY/5);// modify by rongxiao.deng for bug 576583
